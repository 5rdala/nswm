...
void WindowManager_GrabKeys(WindowManager *wm) {
  ...
  for (int i = 0; i < NUM_WORKSPACES; i++) {
    ...
    XGrabKey(wm->dpy, code, SUPER, wm->root, True, GrabModeAsync,
             GrabModeAsync);
  }
}
...
void WindowManager_run(WindowManager *wm) {
  ...
  while (!XNextEvent(wm->dpy, &ev)) {
    if (ev.type == KeyPress) {
      ...
      KeySym sym = XkbKeycodeToKeysym(wm->dpy, ke->keycode, 0, 0);
      if ((ke->state & SUPER) && sym == XK_Return) {
      ...
      XMapWindow(wm->dpy, map->window);
    } else if (ev.type == UnmapNotify) {
      XUnmapEvent *unmap = &ev.xunmap;
      Window w = unmap->window;
